...
def plot_model_history(model_history):
    ...
    fig.savefig('resnet32_reduce_dimension.png')
    plt.show()
...
class Resnet32:
    ...
    def build_model(height, width, depth, classes, stages, num_filters, reg, bnEps, bnMom): 
        ...
        x = Conv2D(num_filters[0], (5, 5), use_bias = False, kernel_regularizer = l2(reg))(x)
        ...
        for i in range(0, len(stages)):
            ...
            for j in range(0, stages[i] - 1):
                ...
                if j%2 == 0:
                    ...
                    x = Resnet32.resnet(x, num_filters[i+1], (1, 1), channel_dimension, reduce_dimension = False, reg = reg, bnEps = bnEps, bnMom = bnMom)
        ...
        x = Activation("relu")(x)
        print("accessed 7")
        x = MaxPooling2D(2, 2)(x) # AveragePooling2D(8, 8)
        print("accessed 8")
...
if __name__ == '__main__':
    ...
    with open("resnet32_reduce_dimension.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("resnet32_reduce_dimension.h5")
